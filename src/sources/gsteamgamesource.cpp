...
static bool parseKeyValueSequence(const QList<Token>& tokens, int& index,
                                  bool untilBlockClose, QVariantMap& result, QString& errorMsg) {
    result.clear();
...
QVariantMap parseACFFileContent(const QString& acfContent, bool* ok, QString* errorMsg) {
    QList<Token> tokens;
...
void GSteamGameSource::reload()
{
    ...
    else if (!mSourceDirectory.isEmpty())
    {
        ...
        for (const auto &f: QDir(steamappsPath).entryList({"appmanifest_*.acf"}, QDir::Files))
        {
            ...
            bool ok;
            QString error;
            const auto result = parseACFFileContent(data, &ok, &error);
            if (!ok)
            {
